...
并集选择器是“和”的意思，  就是说，只要逗号隔开的，所有选择器都会执行后面样式。
...
5. 如果生成带有类名或者id名字的标签，  直接写 标签.demo  或者  标签#two   tab 键就可以了
...
所有的文档元素（标签）都会生成一个矩形框，我们称之为元素框（element box），它描述了一个文档元素在网页布局汇总所占的位置大小。因此，**每个盒子除了有自己大小和位置外，还影响着其他盒子的大小和位置。**
...
```html
浮动首先创建包含块的概念（包裹）。就是说， 浮动的元素总是找离它最近的父级元素对齐,如下：
<div class="father">
    <div class="son">
    </div>
</div>
son这个子元素最近的父元素是father，因此son浮动的话会在father的内容区间中对齐，如果father设置了内边距，son不会超出内边距的范围。 
```
...
```
由2可以推断出，一个父盒子里面的子盒子，想让它们在同一行上，如果其中一个子级有浮动的，则其他子级都需要浮动。
```
...
静态定位唯一的用处就是取消定位。比如有一个盒子使用的absolute（绝对定位），将其改成static静态定位后就能取消它的定位了。
...
对元素设置相对定位后，可以通过**边偏移**属性改变元素的位置，但是它在文档流中的位置仍然保留。如下图所示，即是一个相对定位的效果展示：
...
绝对定位是将元素依据最近的已经定位（静态定位除外的所有定位模式）的父元素（或祖先）为基准进行定位。 
...
如果我们想让元素“压在”其他元素上边，就应该使用绝对定位，因为绝对定位不会占据位置，如果使用相对定位，虽然可以通过边偏移来压在其他元素上，但是它偏移之前的位置是会保留的，这样会在盒子中留下一片空白的区域。

现在有个例子，例如某个栏会跟随页面的滑动也跟着滑动，那么此时这个栏一定要用相对定位，因为这个栏要占据位置，因为如果使用绝对定位不占据位置的话，那么这个栏的位置就会被其他元素占据，结果是这个栏始终会覆盖在其他元素上，这并不是我们想要的。我们希望的是它虽然能跟着页面滑动，但它应该有个原始位置，在原始位置应该有自己的位置。但是如果使用绝对定位的话，它就没有自己的位置了，不管到哪它都会“压着”别人。
...
普通的盒子（有宽度）是左右margin 改为 auto就可， 但是对于加了绝对定位、浮动的盒子就无效了
...
跟浮动一样， 元素添加了**绝对定位**或者**固定定位**之后， 元素模式也会发生转换， 都转换为**行内块级**元素，因此如果行内元素添加了**绝对定位**、**固定定位**或**浮动**后，可以不用转换模式，直接给高度和宽度就可以了。
...
如果某个行内块元素外边套了一个没设置高度的盒子，那么该盒子是随着内容被撑开的。我们知道，图片、表单等行内块元素默认是和文字的基线对齐，那么其实文字的基线和底线之间的距离 + 文字下边距 会形成一个空白的缝隙，如图 ：

<img src="media/fengxi.png" />
...
（3）使用text-overflow: ellipsis让溢出部分用省略号显示
...
图所示为网页的请求原理图，当用户访问一个网站时，需要向服务器发送请求，网页上的每张**图像**都要经过一次请求才能展现给用户。
